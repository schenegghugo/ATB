...
// shove into a wall) detonates it early. onDeath is a radius-4, 25-damage blast
...
Entity makeBomb() {
    ...
    e.onDeath = Spell{"detonation", 0, 0, 0, false, TargetShape::Circle, 4, 0,
                      {Effect{Effect::Type::Damage, 25, {}, {}, {}}}};
    return e;
}
...
// "blocker" — a mobile body whose one trick is a self-centred cross pull that
// only reaches the adjacent tiles: anyone it catches is slammed against its
// bulk for collision damage. A wall of a creature you walk into the enemy.
Entity makeBlocker() {
    Spell drag{"Drag", 3, 0, 0, true, TargetShape::Cross, 1, 2,
               {Effect{Effect::Type::Pull, 2, {}, {}, {}}}};
    return makeSummon("blocker", 18, 4, 4, std::move(drag));
}
...
Entity makeHealer() {
    ...
    return makeSummon("healer", 10, 2, 6, std::move(heal));
}

// "brute" — a bruiser-lite that strikes the nearest foe.
Entity makeBrute() {
    Spell strike{"Strike", 3, 1, 2, true, TargetShape::Single, 0, 0,
                 {Effect{Effect::Type::Damage, 5, {}, {}, {}}}};
    return makeSummon("brute", 12, 4, 5, std::move(strike));
}
